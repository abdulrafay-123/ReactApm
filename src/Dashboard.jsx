...
const Dashboard = () => {
  ...
  const DirectedGraph = React.memo(({ nodes, edges }) => {
    ...
    {renderTile('Alerts', {Count: alertsData['Alerts']}, 'Alerts', 'Current active alerts and their details.', <FaBell />)}
            ...
            <div className="graphs">
              <div className="graph" onClick={() => handleOpenGraph('latencyChart')}>
              ...
              <div className="graph" onClick={() => handleOpenGraph('serviceMap')} >
                            <h3>Service Map</h3>
                            <DirectedGraph nodes={nodes} edges={edges} onNodeSelect={handleNodeSelect} />
                        </div>
              <div className="graph" onClick={() => handleOpenGraph('throughputChart')}>
              ...
              <div className="graph" onClick={() => handleOpenGraph('errorsChart')}>
                <h3>Errors</h3>
  ...
  {activeGraph === 'latencyChart' && (
    <LineChart data={processedLatencyData} chartId="latencyChartModal" width={700} isModalOpen={true} height={400} />
  )}
  {activeGraph === 'serviceMap' && (
                    <div style={{ width: '100%', height: '500px', overflow: 'hidden' }}>
                        <DirectedGraph nodes={nodes} edges={edges} onNodeSelect={handleNodeSelect} />
                    </div>
                )}
  {activeGraph === 'throughputChart' && (
    <LineChart data={processedThroughputData} chartId="throughputChartModal" width={700} isModalOpen={true} height={400} />
  )}
  {activeGraph === 'errorsChart' && (
    <BarChart data={processedThroughputData} chartId="errorsChartModal" isLatencyChart={false} isModal={true} />
  )}
</Modal>
